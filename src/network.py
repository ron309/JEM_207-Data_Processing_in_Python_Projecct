...
def run_network_bundle(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    gas_types = list(df0.type.unique())
    measures = ["primaryValue", "netWgt"]
    for gas_type in gas_types:
        for measure in measures:
            tables[f"top_centralities_{gas_type}_{measure}_2024.csv"] = build_centralities_df(graphs=graphs, 
                                                            gas_type=gas_type, measure=measure, year=2024)
